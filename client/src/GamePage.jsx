...
const GamePage = () => {
  ...
  const prevClosedDataRef = useRef(null);
  ...
  useEffect(() => {
    if (gameState?.closedData && !prevClosedDataRef.current) {
      setAnnouncement({
        msg: `${gameState.closedData.closingPlayer.name} takart!`,
        color: 'orange'
      });
      if (announcementTimeoutRef.current) clearTimeout(announcementTimeoutRef.current);
    ...
    prevClosedDataRef.current = gameState?.closedData || null;
  }, [gameState?.closedData]);
  ...
  const handleTrumpClick = () => {
    if (gameState.isMyTurn && matchResult === null && !gameState.closedData) {
        ...
        {!gameState.closedData ? (
          gameState.deckCount > 0 ? (
            <>
              ...
              {gameState.trumpCard && (
                <div className="absolute top-1/2 -translate-y-1/2 left-12 sm:left-16 md:left-20 lg:left-24 w-28 sm:w-32 md:w-36 lg:w-[160px] aspect-[130/234] rotate-90">
                  <img
          ...
          <div className="relative w-full h-full">
            ...
            <img
              src={`/cards/${gameState.trumpCard.suit}_${gameState.trumpCard.type}.png`}
              alt="Takaró Adu kártya"
              className="absolute top-1/2 -translate-y-1/2 w-28 sm:w-32 md:w-36 lg:w-[160px] aspect-[130/234] object-cover border-2 border-[#D39696] rounded-xl shadow-[0_15px_30px_rgba(0,0,0,0.6)] pointer-events-none transform rotate-[75deg] translate-x-4 sm:translate-x-6"
              style={{ zIndex: 20 }}
            />

            <div className="absolute top-[calc(50%+100px)] sm:top-[calc(50%+120px)] left-0 sm:left-4 z-[30] bg-[#D39696]/95 backdrop-blur-sm border border-white/40 text-white text-xs sm:text-sm font-bold px-4 py-2 rounded-xl shadow-lg whitespace-nowrap">
              Takaró: {gameState.closedData.closingPlayer.name}
            </div>
          </div>
        )}
      </div>
      ...
      {gameState.wonCards && gameState.wonCards.length > 0 && (
        <div className="absolute bottom-6 right-6 z-40 group flex flex-col items-end">
          <div className="absolute bottom-full right-0 mb-4 p-4 bg-white/95 backdrop-blur-md border-2 border-[#D39696]/30 rounded-2xl shadow-2xl opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 pointer-events-none group-hover:pointer-events-auto origin-bottom-right w-max max-w-[85vw] sm:max-w-[400px] z-50">
            <h3 className="text-[#D39696] font-bold text-sm uppercase tracking-widest mb-3 border-b-2 border-[#D39696]/20 pb-1">Ütéseid</h3>
              ...
              {Array.from({ length: Math.floor(gameState.wonCards.length / 2) }).map((_, trickIndex) => {
                ...
                return (
                  <div key={trickIndex} className="relative flex w-16 sm:w-20 hover:scale-110 transition-transform cursor-default">
                    <img src={`/cards/${card1.suit}_${card1.type}.png`} className="w-12 sm:w-14 aspect-[130/234] object-cover rounded-lg shadow-md border border-gray-300 z-10 -rotate-3" alt="Ütés 1" />
                    {card2 && <img src={`/cards/${card2.suit}_${card2.type}.png`} className="absolute left-4 sm:left-6 w-12 sm:w-14 aspect-[130/234] object-cover rounded-lg shadow-xl border border-gray-300 z-20 rotate-6" alt="Ütés 2" />}
                  </div>
                );
              })}
            </div>
          ...
          <div className="relative cursor-pointer hover:-translate-y-2 transition-transform duration-300">
            <img src="/cards/face_down.jpg" className="absolute top-1 left-1 w-20 sm:w-24 aspect-[130/234] object-cover border-2 border-gray-300 rounded-xl shadow-md z-10" alt="Pakli" />
            <img src="/cards/face_down.jpg" className="absolute top-0.5 left-0.5 w-20 sm:w-24 aspect-[130/234] object-cover border-2 border-gray-300 rounded-xl shadow-md z-20" alt="Pakli" />
            <img src="/cards/face_down.jpg" className="relative w-20 sm:w-24 aspect-[130/234] object-cover border-2 border-gray-300 rounded-xl shadow-xl z-30" alt="Pakli" />
          </div>
        </div>
      )}
        ...
        <div className="flex -space-x-8 md:-space-x-12 hover:space-x-1 md:hover:space-x-2 transition-all duration-300 ease-in-out px-4 md:px-10 items-end">
          {gameState.myHand.map((card, i) => {
            ...
            return (
              <div
                key={i}
                className={`relative w-28 sm:w-32 md:w-36 lg:w-[160px] aspect-[130/234] transition-all duration-300 translate-y-[50%] ${gameState.isMyTurn && matchResult === null ? 'cursor-pointer hover:-translate-y-2 md:hover:-translate-y-6 hover:z-50' : 'cursor-not-allowed opacity-80'}`}
              ...
              >
                <img
                  ...
                  className={`w-full h-full object-cover rounded-xl transition-all duration-300 ${isThisCardInvalid ? 'border-4 border-red-600 animate-shake shadow-[0_0_30px_rgba(220,38,38,1)]' : 'border-2 border-gray-300 shadow-lg hover:shadow-2xl hover:border-[#D39696]'}`}
                />
              </div>
            );
          })}
